...
<div>    
    ...
    <div class="poster-wrapper">
        <div class="poster-avatar">
            <el-avatar :size="110" :src="p_avatar_url" />
        </div>
        ...
        <div class="reviews-wrapper">
            <div  v-for="(comment, index1) in comments" :key="index1" class="one-comment">
                <div class="com-avatar">
                    <el-avatar :size="60" :src="comment.poster_avatar" />
                </div>
                <div class="comment-and-response">
                    ...
                    <div v-for="(reply, index2) in comments_of_comments[index1]" :key="index2" class="one-reply">
                        <div class="rep-avatar">
                            <el-avatar :size="30" :src="reply.poster_avatar" />
                        </div>
...
export default {
    ...
    methods:{
        initializeData(){
            var that = this;
            that.reader.reader_id = "002";      //这个暂时不知道怎么获得，先写死
            that.article_id = "12345";          //日志id，先写死
            ...
            axios.get('/Journal/JournalDetails/getUserInteraction?id='+ that.reader.reader_id + '&article_id=' + that.article_id)
            .then(res =>{
                that.reader.is_like = res.data.is_like;
                that.reader.is_collect = res.data.is_collect;
                that.reader.is_followed = res.data.is_followed;
            });
            axios.get('/Journal/JournalDetails/getDetails?article_id='+ that.article_id)
            .then(res=>{
               that.cover_url = res.data.cover_url;
               that.title = res.data.title;
               that.p_avatar_url = res.data.poster_avatar;
               that.poster_name = res.data.poster_name;
               that.poster_id = res.data.poster_id;
               that.read_num = res.data.read_num + 1;       //浏览数，+1
               that.kudos_num = res.data.kudos_num;
               that.collect_num = res.data.collect_num;
               that.post_date = res.data.post_date;
               that.is_followed_display = (that.reader.reader_id !== that.poster_id);
               that.details_tabs = res.data.tabs;
               that.details_body = res.data.body; 
            }); 
            axios.get('/Journal/JournalDetails/getComments?id='+ that.reader.reader_id + '&article_id=' + that.article_id)
            .then(res=>{
                that.comments = res.data.comments;
                that.comments_of_comments = res.data.comments_of_comments;
            })
        },
        ...
        changeLikeStatus(){
            ...
            if(this.reader.is_like) this.kudos_num++;  //点赞
            else this.kudos_num--;                    //取消点赞 
            */
            axios.post('/Journal/JournalDetails/changeLikeStatus/',{
                is_like: !that.reader.is_like,
        ...
        changeCollectStatus(){
            ...
            if(this.reader.is_collect) this.collect_num++;
            else this.collect_num--;
            */
            axios.post('/Journal/JournalDetails/changeCollectStatus/',{
                is_collect: !that.reader.is_collect,
                user_id: that.reader.reader_id,
                article_id: that.article_id,
            })
            .then(res => {
        ...
        change_com_LikeStatus(index){
            ...
            if(this.comments[index].is_like)
                ...
                this.comments[index].post_likes--;
            */
            axios.post('/Journal/JournalDetails/changeCommentsLikeStatus/',{
                is_like: !that.comments[index].is_like,
                user_id: that.reader.reader_id,
                comment_id: that.comments[index].comment_id,
            })
            .then(res => {
                if(res.data.status){
                    that.comments[index].is_like= !that.comments[index].is_like;
                    that.comments[index].post_likes = res.data.post_likes;
                }
            })
            .catch(error => {
                alert('操作失败！');
        ...
        change_rep_LikeStatus(index1, index2){
            ...
            */
            axios.post('/Journal/JournalDetails/changeCommentsLikeStatus/',{
                is_like: !that.comments_of_comments[index1][index2].is_like,
                user_id: that.reader.reader_id,
            ...
            .then(res => {
                if(res.data.status){
                    ...
                    that.comments_of_comments[index1][index2].post_likes = res.data.post_likes;
                }
            })
            .catch(error => {
                alert('操作失败！');
        ...
        post_comment_response(index1, index2){
            ...
            if(index1 == -1){           //评论日志
            axios.post('/Journal/JournalDetails/postComments/',{
            ...
            axios.post('/Journal/JournalDetails/postResponse/',{
                poster_id: that.reader.reader_id,
                replied_id: id,
                content: that.reader.post_response,
                post_time: formatDateTime(now_time),
                post_likes: 0,
            })
            .then(res => {
        ...
        delete_com_reply(index1, index2){
            ...
            if(index2 == -1){       // c中
                axios.post('/Journal/JournalDetails/deleteComments/',{
